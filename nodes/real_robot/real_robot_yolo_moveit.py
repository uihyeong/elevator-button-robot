...
class RealRobotYoloMoveIt(Node):
    def __init__(self):
        super().__init__('real_robot_yolo_moveit')
        ...
        self.get_logger().info('실제 로봇 YOLO MoveIt2 시작!')
                ...
                try:
                    ...
                    if cls == self.target_button and conf > 0.7 and not self.button_pressed and self.ready and not self.task_done:
                        ...
                        threading.Thread(
                            ...
                            args=(X - 0.075, Y, Z), daemon=True).start()  # 버튼 표면 7.5cm 앞에서 멈춤

                except Exception as e:
                    self.get_logger().warn(f'TF 변환 실패: {e}')

        cv2.imshow('Real Robot YOLO MoveIt2', frame)
        cv2.waitKey(1)
