...
class RealRobotCartesian(Node):
    def __init__(self):
        super().__init__('real_robot_cartesian')
        ...
        self.get_logger().info('실제 로봇 Cartesian Path 노드 시작!')
    ...
    def depth_callback(self, msg):
        with self.lock:
            ...
            self.depth_image = depth_raw.astype(np.float32) / 1000.0  # mm → m
    ...
    def image_callback(self, msg):
            ...
            for box in result.boxes:
                ...
                try:
                    ...
                    if (cls == self.target_button and conf > 0.7
                        ...
                        self.get_logger().info(f'{cls} 감지! Cartesian path 계획 시작')
        ...
        cv2.imshow('Real Robot Cartesian', frame)
        cv2.waitKey(1)

    # ─── Cartesian Path ───────────────────────────────────────────────────

    def get_current_eef_pose(self):
        """현재 end_effector_link 위치를 world 프레임으로 반환"""
        try:
    ...
    def move_cartesian(self, target_x, target_y, target_z):
        ...
        self.get_logger().info(f'Cartesian path 성공 ({fraction:.1%}) → 실행 시작')
